...
| Explain agent action | `python3 .aletheia/bin/action.py explain truth.validate --json` | Read one `actions.json` action contract | Done | Action IDs include `truth.validate`, `truth.history_audit`, `truth.preflight`, `truth.checkpoint.dry_run`, `truth.orphan.*`, `truth.bootstrap.*.inspect`, and `capability.audit`. |
...
| Recommend next agent actions | `python3 .aletheia/bin/action.py next --json` | Read recommended action IDs from `actions.json` | Done | Recommended IDs include `truth.orient.runtime`, `truth.status`, `truth.preflight`, `truth.validate`, `truth.history_audit`, and `truth.checkpoint.dry_run`; preflight also emits these action IDs for hook-free hosts. |
...
| Audit Git truth history | `python3 .aletheia/bin/history_audit.py --json` or `truth.history_audit` | Parse Git history for AletheiaOS traceability trailers, `AIOS-Node-State: stable` claims, implementation links, and transition errors | Done | Use before claiming stable truth-tree growth is reviewable from Git history. |
...
| Audit Git truth history | `history_audit.py --json` | Reconstruct stable-node claims, implementation links, and traceability errors from commit trailers. |
