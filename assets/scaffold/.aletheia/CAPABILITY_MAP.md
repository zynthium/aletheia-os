...
| Orphan incubator | `truth_record.py create orphan` | `truth_record.py list/show orphan`, `orient.py`, `context_pack.py`, `system_context.py`, `status.py`, `overview.py` | `truth_record.py update orphan <id> --status ...` or direct `ORPHANS.yaml` edits for broad changes | `truth_record.py archive orphan <id>` | Used for unmounted observations, candidate theories, and weak claims that should not pollute the main tree. |
