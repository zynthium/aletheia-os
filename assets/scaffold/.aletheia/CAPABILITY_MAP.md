...
| Active state and state files | scaffold creates files | `truth_record.py show active-state current`, `orient.py`, `context_pack.py`, `status.py`, `overview.py` | `truth_record.py update active-state current --section ...` or edit files | `truth_record.py archive active-state current` | Validate checks critical TBD markers after bootstrap. |
